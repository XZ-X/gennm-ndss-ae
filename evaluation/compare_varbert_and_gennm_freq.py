...
print()
# build training name frequency

train_name_freq = {}
for entry in tqdm(train_ds['train'], desc="build training name freq", total=len(train_ds['train'])):
    rsp = eval(entry['response'][2:])
    for dummy_name, gt_name in rsp.items():
        if dummy_name is None:
            continue
        if dummy_name.startswith("sub_"):
            continue
        if gt_name not in train_name_freq:
            train_name_freq[gt_name] = 0
        train_name_freq[gt_name] += 1

df_all['gt_name_freq'] = df_all['gt_varname'].apply(lambda x: train_name_freq.get(x, 0))

non_overlap_all = df_all[df_all["overlapped"] == False]

freq_precision_map = {}
freq_list = [-1, 0, 10, 100, 1000, 999999]
for k in range(1, len(freq_list)):
    lower_bound = freq_list[k-1]
    upper_bound = freq_list[k]
    non_overlap_all_k = non_overlap_all[(non_overlap_all['gt_name_freq'] > lower_bound) & (non_overlap_all['gt_name_freq'] <= upper_bound)]
    avg_precision = non_overlap_all_k['gennm_precision'].mean()
    avg_varbert_precision = non_overlap_all_k['varbert_precision'].mean()
    freq_precision_map[upper_bound] = (avg_precision, avg_varbert_precision)

for freq, (gn_pr, vb_pr) in freq_precision_map.items():
    print(f"Frequency range: {freq}, GenNM precision: {gn_pr}, VarBERT precision: {vb_pr}")
