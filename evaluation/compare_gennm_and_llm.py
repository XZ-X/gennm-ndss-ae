...
parser.add_argument("--gennm_evaluated", type=str, default="", help="path to gennm ret evaluated")
parser.add_argument("--llm_evaluated", type=str, default="", help="path to LLM ret evaluated")
...
gennm_ret_evaluated = [json.loads(l) for l in open(args.gennm_evaluated, "r")]
llm_ret_evaluated = [json.loads(l) for l in open(args.llm_evaluated, "r")]
...
for entry in gennm_ret_evaluated:
...
llm_evaluated_by_prog_func_var_varbert_format = {}
for entry in llm_ret_evaluated:
    prog = entry["prog_name"].split("_")[0]
    func = entry["func_name"]
    if not func.startswith(FUNC_PREFIX):
        continue
    ...
    if 'sub' in FUNC_PREFIX:
        ...
        func_addr_str = f"{func_addr:08x}"    
    var_name = entry["varname"]
    llm_evaluated_by_prog_func_var_varbert_format[(prog, func_addr_str, var_name)] = entry

varbert_evaluated_by_prog_func_var = {}
for entry in varbert_ret_evaluated:
    prog = entry["prog_name"].split("_")[0]
    func = entry["func_name"]
...
) & set(varbert_evaluated_by_prog_func_var.keys()) & set(llm_evaluated_by_prog_func_var_varbert_format.keys())
...
llm_overlapped_entries = []
for prog_func_var in overlapped_prog_funcs_var:
    llm_entry = llm_evaluated_by_prog_func_var_varbert_format[prog_func_var]
    prog, func, var = prog_func_var
    if (prog, func) not in no_overlap_varbert_format_set:
        overlapped = True
    else:
    ...
    llm_overlapped_entries.append(
        {
            "prog_name": prog,
            "func_name": func,
            "varname": var,
            "gt_varname": llm_entry["gt_varname"],
            "pred_name": llm_entry["pred_name"],
            "precision": llm_entry["precision"],
            "recall": llm_entry["recall"],
            "overlapped": overlapped,
            "proj_in_train": proj_in_train,
            "proj_category": proj_category,
            "proj_name": proj_entry["project_name"],
...
df_llm = pd.DataFrame(llm_overlapped_entries)
...
df_all = df_gennm.merge(
    df_llm, on=["prog_name", "func_name", "varname"], how="inner"
)
...
#     ['gennm_precision', 'gennm_recall', 'llm_precision', 'llm_recall']
# ].mean()
...
overall = df_to_analyze.groupby(["prog_name"]).agg(
    {
        "gennm_precision": "mean",
        "gennm_recall": "mean",
        "llm_precision": "mean",
